...
const CardsSectionItem: React.FC<types.Card & types.StackbitFieldPath & { titleTag?: 'h3' | 'h2' }> = (props) => {
    let { title, text, image, actions = [], titleTag = 'h3', 'data-sb-field-path': fieldPath, abstractReferenceContent } = props;

    //Override the content with what is coming from contentful.
    //TODO: Update parsing logic to preseve types
    
    if(abstractReferenceContent) {
        title = abstractReferenceContent.fields.title;
        text = abstractReferenceContent.fields.text;
        image = abstractReferenceContent.fields.image.fields.file;
    }
